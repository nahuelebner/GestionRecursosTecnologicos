...
{
    public class GestorRegistrarMantenimiento : ISujetoNotificacion
    {
        #region Atributos
        ...
        private List<string> formasNotificacion;
        ...
        internal List<IObservador> Observadores { get => observadores; set => observadores = value; }
        public List<string> FormasNotificacion { get => formasNotificacion; set => formasNotificacion = value; }
        ...
        #region Métodos del Observer

        // recibe una lista de booleanos según los checkbox seleccionados
        // y obtiene la lista de mails o teléfonos según corresponda.
        public void tomarFormaNotificacion(List<bool> listaBooleanos)
        {
            contactos = new List<string>[2];
            formasNotificacion = new List<string>();
            if (listaBooleanos[0])
            {
                formasNotificacion.Add("Mail");
                contactos[0] = new List<string>();
                foreach (PersonalCientifico cientifico in cientificosANotificar)
                {
                    contactos[0].Add(cientifico.CorreoElectronicoInstitucional);
                }
            }
            if (listaBooleanos[1])
            {
                formasNotificacion.Add("WhatsApp");
                contactos[1] = new List<string>();
                foreach (PersonalCientifico cientifico in cientificosANotificar)
                {
                    contactos[1].Add("3516548972");
                }
            }
        }
        public void enviarNotificaciones() // le cambiamos el nombre para que abarque
                                           // todos los posibles tipos de notificacion.
                                           // ex generarMailsNotificacion
        {
            obtenerObservadores();
            notificar();
        }
        ...
        {
            foreach (string forma in formasNotificacion)
            {
                if (forma == "Mail")
                {
                    IObservador observador = new InterfazMail(); 
                    suscribir(observador);
                }
                if (forma == "WhatsApp")
                {
                    IObservador observador = new InterfazWhatsApp();
                    suscribir(observador);
                }
            }
        }
        ...
        public void notificar()
        {
            ...
            {
                observador.generarMailsNotificacion(razonMantenimiento, contactos);
            }
        }
